                ...
                <form method="POST" action="{{ route('peminjaman.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="id_member" name="id_member" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required autofocus>
                            <option value="" disabled {{ old('id_member') ? '' : 'selected' }}>Pilih Member</option>
                    ...
                    <div>
                        ...
                        <select id="id_buku" name="id_buku" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>
                            <option value="" disabled {{ old('id_buku') ? '' : 'selected' }}>Pilih Buku</option>
                            @foreach($buku as $b)
                    ...
                    <div>
                        ...
                        <x-text-input id="tgl_pinjam" class="block mt-1 w-full" type="date" name="tgl_pinjam" :value="old('tgl_pinjam')" required />
                        <x-input-error :messages="$errors->get('tgl_pinjam')" class="mt-2" />
                    </div>

                    <div>
                        <x-input-label for="tgl_kembali" :value="__('Tanggal Kembali')" />
                        <x-text-input id="tgl_kembali" class="block mt-1 w-full" type="date" name="tgl_kembali" :value="old('tgl_kembali')" />
